...
export const Pricing = () => {
    ...
    return (
        <section className="bg-[#f0ebe1] px-6 py-24 theme-bg min-h-screen transition-colors duration-300 font-serif">

            <div className="text-center mt-14 mb-10">
                <p className="font-mono text-xs tracking-[0.18em] text-[#8a7f70] uppercase mb-4">
                    Simple, transparent pricing
                </p>
                <h2 className="text-4xl sm:text-5xl font-extrabold text-[#1a1a2e] dark:text-gray-300 leading-tight tracking-tight mb-5">
                    Pick Your Plan
                </h2>
                ...
                <div className="inline-flex items-center gap-3 bg-[#e8e0d2] rounded-full p-1.5">
                    <button
                        ...
                        className={`px-5 py-2 rounded-full font-mono text-xs tracking-widest uppercase font-semibold transition-all duration-200 ${!yearly ? "bg-[#1a1a2e] text-[#f0ebe1]" : "bg-transparent text-[#8a7f70]"
                            }`}
                    >
                        Monthly
                    </button>
                    <button
                        ...
                        className={`px-5 py-2 rounded-full font-mono text-xs tracking-widest uppercase font-semibold transition-all duration-200 flex items-center gap-2 ${yearly ? "bg-[#1a1a2e] text-[#f0ebe1]" : "bg-transparent text-[#8a7f70]"
                            }`}
                    >
                        Yearly
...
function PricingCard({ plan, yearly }: { plan: Plan; yearly: boolean }) {
    ...
    return (
        ...
        >
            ...
            <div className="mb-6">
                <span
                    className={`font-mono text-[11px] tracking-[0.14em] uppercase font-bold px-3 py-1 rounded-full ${h ? "text-[#c8b89a] bg-[rgba(200,184,154,0.12)]" : "text-[#8a7f70] bg-[#f5f0e8]"
                        }`}
                >
                    {plan.tag}
                </span>
            ...
            {/* Name */}
            <h3
                className={`text-2xl font-extrabold tracking-tight mb-2 ${h ? "text-[#f0ebe1]" : "text-[#1a1a2e]"
                    }`}
            >
                {plan.name}
            </h3>
            ...
            <ul className="space-y-3 mb-8">
                {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5">
                        <span
                            className={`shrink-0 w-[18px] h-[18px] rounded-full flex items-center justify-center mt-0.5 ${h ? "bg-[rgba(200,184,154,0.15)]" : "bg-[#f0ebe1]"
                                }`}
                        >
            ...
            <button
                onClick={handleCTA}
                disabled={loading}
                className={`w-full py-3.5 rounded-2xl font-mono text-xs font-bold tracking-[0.14em] uppercase transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed ${h
